(function ($, window, document, undefined) {
    ...
    function modal(params) {
    	// not allow overlap 
    	if( $('body').find('div.notOverlap').length > 0){
    		return false;
    	}
    	params = params || {};
        var modalHTML = '';
        ...
        modalHTML = '<div class="modal ' + noButtons + ' ' + ( params.notAllowrOverlap ? 'notOverlap ' : '' ) + (params.cssClass || '') + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
    ...
    function openModal(modal) {
        ...
        if (isModal) {
            modal.show();
            modal.css({
            	//@hyjung 20181102 scroll 위치에 따른 confirm 위치 수정 
            	marginTop: -Math.round(modal.outerHeight() / 2) +($(window).scrollTop()) + 'px'
            });
        }
    ...
    var modalTitle = '';
    var modalButtonOk = '확인';
    var modalButtonCancel = '취소';
    ...
    $.extend({
        ...
        confirmYn: function (text, title, callbackOk, callbackCancel) {
            if (typeof title === 'function') {
            ...
            return modal({
            	notAllowrOverlap : false,
            	text: text || '',
                title: typeof title === 'undefined' ? modalTitle : title,
                buttons: [
                	{text: '예', bold: false, onClick: callbackOk},
                    {text: '아니오', onClick: callbackCancel}
                ]
            });
        },
        confirm: function (text, title, callbackOk, callbackCancel) {
            ...
            return modal({
                ...
                title: typeof title === 'undefined' ? modalTitle : title,
                buttons: [
                	{text: modalButtonOk, bold: false, onClick: callbackOk},
