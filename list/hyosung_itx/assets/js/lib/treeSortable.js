...
function TreeSortable(options) {
                ...
                <div class="branch-wrapper">
                    <div class="left-sidebar">
                        <div class="${self.cleanSelector(dragHandlerSelector)}">
                            <i class="ico icoDrag">드래그</i>
                        </div>
                        <div class="folder">
                            <i class="ico icoFol">폴더</i>
                        </div>
                        <div class="inpBox">
                            <input type="text" class="branch-title" value="${title}" />
                            <button type="button" class="delBtn">
                                <i class="ico icoDel"></i>
                            </button>
                        </div>
                    </div>
                    <div class="right-sidebar">
                        <button type="button" class="btn sm outline add-child" data-tippy-content="Add a new child">
                            <i class="ico icoChild">하위</i>하위
                        </button>
                        <button type="button" class="btn sm outline add-sibling" data-tippy-content="Add a new sibling">
                            <i class="ico icoSibling">동급</i>동급
                        </button>
                        <button type="button" class="btn sm outline remove-branch" data-tippy-content="Remove Branch">
                            <i class="ico icoRemove">삭제</i>삭제
                        </button>
                    </div>
                </div>
            </div>
    ...
    $(document).on('click', '.folder', function(e) {
        ...
        // 클릭한 버튼이 속한 branch li
        const $branch = $(this).closest('.tree-branch');

        // branch의 childrenBus
        const $childrenBus = $branch.find('.children-bus').first();

        // 접기/펼치기 토글
        $childrenBus.slideToggle(200); // 부드럽게 애니메이션
