// 공장 : /showroom/factory.html
function showroom5() {
    // 초기 진입시 메인 실행
    const sr5Factory = $('.sr5-factory');
    const srSw = sr5Factory.find('.showroom-screen-wrap');
    const srSwArr = [
        sr5Factory.find('.showroom-screen-wrap0'),
        sr5Factory.find('.showroom-screen-wrap1'),
        sr5Factory.find('.showroom-screen-wrap2'),
        sr5Factory.find('.showroom-screen-wrap3'),
    ];
    const srPointer = sr5Factory.find('.sr-pointer');
    ...
    startTimers.push(
        setTimeout(() => {
            sr5Factory.removeClass('default').addClass('main');
    ...
    srPointer.on('click', function () {
        ...
        sr5Factory.removeClass('main');
        sr5Factory.addClass(screenClass + ' step1 step1-1');

        if (typeof showroomData !== 'undefined') {
            setPageContent('sr5-factory', screenClass, showroomData);
    ...
    const popChkCompleteLink = sr5Factory.find('.popup-check-complete-wrap');
    ...
    const btnScreen1End1 = sr5Factory.find('.button-swiper-bottom-screen1');
    ...
    const btnScreen1End2 = sr5Factory.find('.button-swiper-bottom-screen2');
    ...
    const btnScreen1End3 = sr5Factory.find('.button-swiper-bottom-screen3');
    ...
    btnScreenWrap.on('click', function () {
        localStorage.setItem('previousPage', 'factory.html'); // 이전페에지 가기 스토리지 저장
...
function sr5InfoViewPopup() {
    const sr5InfoViewPopupWrap = $('.sr5-info-view-popup-wrap');
    const sr5InfoPopupWrap = sr5InfoViewPopupWrap.find('.sr5-info-popup-wrap');
    const sr5InfoButtonWrap = sr5InfoViewPopupWrap.find('.sr5-info-button-wrap');
    const sr5ButtonInfoView = $('.sr5-button-info-view');
    const sr5PopupViewItem = sr5InfoPopupWrap.find('.sr5-popup-view-item');
    const sr5IconViewClose = sr5PopupViewItem.find('.sr5-icon-view-close');
    const sr5PersonSc = $('.sr5-person-sc3-st2');
    const sr5PersonSc3St2Obj = sr5PersonSc.find('.sr5-person-sc3-st2-obj');
    const totCnt = 100;
    let choiceTimers = [];

    sr5ButtonInfoView.on('click', function () {
        const thisB = $(this);
        const sr5InfoButtonItem = thisB.closest('.sr5-info-button-item');
        const thisItemIdx = sr5InfoButtonItem.index();
        const index = sr5InfoButtonItem.index();
        ...
        if (sr5InfoButtonItem.hasClass('active')) {
            sr5InfoButtonItem.removeClass('active');
            sr5InfoPopupWrap.removeClass('active');
            sr5PopupViewItem.eq(index).removeClass('active');

            for (let i = 1; i <= totCnt; i++) {
                thisB
                    .closest('.sr5-info-view-popup-wrap')
                    .siblings('.sr5-alert-msg-wrap')
                    .removeClass('changeEp' + i);
                thisB
                    .closest('.sr5-info-view-popup-wrap')
                    .siblings('.sr5-person-sc')
                    .removeClass('changeEp' + i);
                thisB
                    .closest('.sr5-info-view-popup-wrap')
                    .siblings('.sr5-item-skew-device-wrap')
                    .removeClass('changeEp' + i);
                thisB
                    .closest('.sr5-info-view-popup-wrap')
                    .siblings('.sr5-targeting')
                    .removeClass('changeEp' + i);
            }
            sr5PersonSc3St2Obj.removeClass('choice');
            thisB
                .closest('.sr5-info-view-popup-wrap')
                .siblings('.sr5-person-sc')
            ...
            sr5InfoButtonWrap.find('.sr5-info-button-item').removeClass('active');
            sr5InfoPopupWrap.removeClass('active');
            sr5PopupViewItem.removeClass('active');
            sr5InfoButtonItem.addClass('active');
            sr5InfoPopupWrap.addClass('active');
            sr5PopupViewItem.eq(index).addClass('active');
            for (let i = 1; i <= totCnt; i++) {
                thisB
                    .closest('.sr5-info-view-popup-wrap')
                    .siblings('.sr5-alert-msg-wrap')
            ...
            choiceTimers.push(
                setTimeout(() => {
                    sr5PersonSc3St2Obj.eq(thisItemIdx).addClass('choice');
                }, 300),
            );
        }
    });

    sr5IconViewClose.on('click', function () {
        const thisB = $(this);
        const sr5PopupViewItem = thisB.closest('.sr5-popup-view-item');
        const index = sr5PopupViewItem.index();

        if (sr5PopupViewItem.hasClass('active')) {
            sr5PopupViewItem.removeClass('active');
            sr5InfoPopupWrap.removeClass('active');
            sr5PopupViewItem.eq(index).removeClass('active');
            sr5InfoButtonWrap.find('.sr5-info-button-item').removeClass('active');
            ...
            sr5InfoPopupWrap.removeClass('active');
            sr5PopupViewItem.removeClass('active');
            sr5PopupViewItem.addClass('active');
            sr5InfoPopupWrap.addClass('active');
            sr5PopupViewItem.eq(index).addClass('active');
            for (let i = 1; i <= totCnt; i++) {
                thisB
