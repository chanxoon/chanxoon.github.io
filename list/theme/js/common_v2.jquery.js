...
$(function() {
    ...
    */
    ...
    if(typeof $.getUrlVars('jsi').jsi != "undefined" && $.getUrlVars('jsi').jsi != null) {
        ...
        setCookie('jsi', jsi, 1);
        // 스크랩 여부 확인
        //$('.detail-scrap-icon-contrl').css('display', 'inline-block');
        //getScrapStatus(jsi, '{{ id }}', 'start');
        //countingViews(jsi, '{{ id }}');
    } else {
        if(typeof getCookie('jsi') != "undefined" && getCookie('jsi') != '' ) {
            //$('.detail-scrap-icon-contrl').css('display', 'inline-block');
            //getScrapStatus(getCookie('jsi'), '{{ id }}', 'start');
            //countingViews(getCookie('jsi'), '{{ id }}');
        }
    }
